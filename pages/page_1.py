...
_store_main_tab         = _make_store("p1_main_tab",         "Comments")
_store_comment_tab      = _make_store("p1_comment_tab",      "Counts")
...
def _sidebar_controls(
    ...
    if is_closed:
        ...
        reference_date = datetime.combine(closed_date, datetime.min.time()) + timedelta(days=1)
    else:
        reference_date = datetime.now()

    non_empty = [df for df in all_dfs if not df.empty]
    if not non_empty:
        return reference_date, is_closed, (datetime.now().date(), datetime.now().date()), []
    ...
    global_date_max = (
        ...
        if is_closed else datetime.now().date()
    )
...
if file_bytes:
    ...
    # ---------------------------------------------------------------------------
        ...
        col2.metric("Insertions", int((filtered_r_df["kind"] == "insertion").sum()) if not filtered_r_df.empty else 0)
        col3.metric("Deletions",  int((filtered_r_df["kind"] == "deletion").sum())  if not filtered_r_df.empty else 0)
