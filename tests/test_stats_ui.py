...
def test_sidebar_controls_open_matter_uses_data_max(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(st.sidebar, "markdown", lambda text: None)
    monkeypatch.setattr(st.sidebar, "toggle", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        st.sidebar,
    ...
    monkeypatch.setattr(
        st.sidebar,
        "slider",
        lambda *args, **kwargs: (date(2024, 1, 1), date(2024, 1, 10)),
    )
    monkeypatch.setattr(
        st.sidebar,
        "multiselect",
    ...
    df = pd.DataFrame(
        {
            "date": [pd.Timestamp("2024-01-01"), pd.Timestamp("2024-01-10")],
            "author": ["Alice", "Bob"],
        }
    )
    ...
    assert not is_closed
    assert date_range == (date(2024, 1, 1), date(2024, 1, 10))
